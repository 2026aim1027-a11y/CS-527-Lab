# CS527 Lab 4 - Multi-Processing Mini-Computer OS

Builds on the Lab 3 vector-extended simulator (`compiler.c`, `opcodes.h`,
the ISA and vector instructions are unchanged) and adds everything Lab 4
asks for on top of it: a `Print` instruction, `NUM_PROCESSORS` (=4)
independent processor/memory instances, and a thin OS layer
(scheduler + shell + loader) that multiplexes tasks across them.
...
```
$ ./sim
[OS] CS527 Lab 4 multi-processing OS - 4 processors.
...
src/memory.c/.h   - per-processor Instruction[]/Data[], hex load/save
