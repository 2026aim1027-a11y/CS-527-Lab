...
#include "compat.h"    /* ci_strncasecmp - portable, no <strings.h> dependency */

#define MAX_LINES   512
/* Lab 5: instruction logical space grew from 256 bytes (Lab 1-4) to
 * 1024 bytes / 256 instructions - see opcodes.h's INSTR_LOGICAL_SIZE
 * comment for why. */
#define MAX_INSTR   (INSTR_LOGICAL_SIZE / 4)
...
static char classify_operand(const char *tok, int *value) {
    ...
    if (is_all_digits(tok)) {
        long v = atol(tok);
        if (v > 255) {
            load_error("constant %ld out of range - constants must be 0-255 "
...
static Instr *emit(void) {
    if (instr_count >= MAX_INSTR) {
        load_error("program too large (max %d instructions / %d bytes of instruction memory)\n",
...
static void load_and_clean(const char *srcfile) {
    ...
    if (!fp) {
        load_error("could not open source file '%s'\n", srcfile);
    }
    char raw[LINE_LEN];
    while (fgets(raw, sizeof(raw), fp)) {
        ...
        if (clean_line_count >= MAX_LINES) {
            load_error("source file too long\n");
        }
...
static void parse_branch(const char *line) {
    ...
    if (*p != '.') {
        load_error("malformed branch instruction '%s' (expected .label)\n", line);
    }
    p++; /* skip '.' */
    ...
    if (code < 0) {
        load_error("unknown branch suffix '%s' in '%s'\n", suffix, line);
    }
...
static int is_print_line(const char *line) {
    return (ci_strncasecmp(line, "print", 5) == 0 && isspace((unsigned char)line[5]));
}

static void parse_print(const char *line) {
    char regTok[64];
    if (sscanf(line + 5, "%63s", regTok) != 1) {
        load_error("malformed Print instruction '%s'\n", line);
    }
    int v;
    char t = classify_operand(regTok, &v);
    if (t != 'x') {
        load_error("'Print <xreg>' expected, got '%s'\n", line);
    }
    Instr *ins = emit();
    ins->opcode = OP_PRINT;
    ins->dest = 0; ins->src1 = 0; ins->src2 = v;
}
...
/* Legacy Read/Write's own example syntax in the handout uses a comma
 * ("Read x1, 0" / "Write x7, 20"); strip a single trailing comma off a
 * token in place so "x1," classifies the same as "x1". */
static void strip_trailing_comma(char *tok) {
...
static void parse_legacy_read(const char *line) {
    ...
    if (sscanf(line + 5, "%63s %63s", varTok, addrTok) != 2) {
        load_error("malformed Read instruction '%s'\n", line);
    }
    strip_trailing_comma(varTok);
    int v1, v2;
    char t1 = classify_operand(varTok, &v1);
    char t2 = classify_operand(addrTok, &v2);
    if (t1 != 'x' || t2 != 'c') {
        load_error("'Read <xreg>, <constant address>' expected, got '%s'\n", line);
    }
    Instr *ins = emit();
...
static void parse_legacy_write(const char *line) {
    ...
    if (sscanf(line + 6, "%63s %63s", varTok, addrTok) != 2) {
        load_error("malformed Write instruction '%s'\n", line);
    }
    strip_trailing_comma(varTok);
    int v1, v2;
    char t1 = classify_operand(varTok, &v1);
    char t2 = classify_operand(addrTok, &v2);
    if (t1 != 'x' || t2 != 'c') {
        load_error("'Write <xreg>, <constant address>' expected, got '%s'\n", line);
    }
    Instr *ins = emit();
...
static void split_at_equals(char *line, char *lhs, char *rhs) {
    char *eq = strchr(line, '=');
    if (!eq) {
        load_error("no '=' found in statement '%s'\n", line);
    }
    *eq = '\0';
    strcpy(lhs, line);
    strcpy(rhs, eq + 1);
...
static void extract_bracket(const char *s, char *out) {
    ...
    if (!open || !close || close < open) {
        load_error("malformed bracket expression '%s'\n", s);
    }
...
static void parse_memory_write(const char *lhs, const char *rhs) {
    ...
    if (valType == 'v') { /* vector store */
        ...
        else { load_error("bad address in '[%s] = %s'\n", addrBuf, rhs); }
        ...
        load_error("right-hand side of a memory write must be a register, got '%s'\n", rhs);
    }
    ins->src1 = 0;
}
...
static void parse_memory_read(const char *lhs, const char *rhs) {
    ...
    if (destType == 'v') {
        ...
        load_error("left-hand side of a memory read must be a register, got '%s'\n", lhs);
    }
}
...
static void parse_data_move_or_copy(const char *lhs, const char *rhs) {
    ...
    if (destType != 'x' && destType != 'v') {
        load_error("invalid destination '%s'\n", lhs);
    }
    Instr *ins = emit();
    ins->dest = destVal;
    if (rhsType == 'c') {
        if (destType != 'x') {
            load_error("data movement of a constant is only defined for x-registers ('%s = %s')\n", lhs, rhs);
        }
        ...
        load_error("cannot assign %s to %s ('%s = %s')\n",
...
static void parse_arithmetic(const char *lhs, char *rhs) {
    ...
    if (sscanf(rhs, "%63s %3s %63s", op1Tok, opChar, op2Tok) != 3) {
        load_error("malformed arithmetic expression '%s = %s'\n", lhs, rhs);
    }
    ...
    if (destType == 'x') {
        if (op1Type != 'x') {
            load_error("first operand of an integer expression must be an x-register ('%s = %s')\n", lhs, rhs);
        }
        ins->src1 = op1Val;
        int isConst = (op2Type == 'c');
        if (!isConst && op2Type != 'x') {
            load_error("bad second operand '%s' in '%s = %s'\n", op2Tok, lhs, rhs);
        }
        ins->src2 = op2Val;
        switch (op) {
            ...
            default:
                load_error("unsupported integer operator '%c' in '%s = %s'\n", op, lhs, rhs);
        }
    } else if (destType == 'v') {
        if (op1Type != 'v') {
            load_error("first operand of a vector expression must be a vector register ('%s = %s')\n", lhs, rhs);
        }
        ins->src1 = op1Val;
        ins->src2 = op2Val;
        if (op != '+' && op != '-' && op != '*') {
            load_error("unsupported vector operator '%c' (only + - * are defined) in '%s = %s'\n", op, lhs, rhs);
        }
        if (op2Type == 'v') {
...
static void parse_line(char *line) {
    ...
    if (is_print_line(line))  { parse_print(line);  return; }
...
void compile(const char *srcfile, const char *outfile) {
    ...
    if (looks_like_bytecode()) {
        ...
        if (!in || !out) {
            load_error("could not copy bytecode file through\n");
        }
        char buf[LINE_LEN];
    ...
    FILE *out = fopen(outfile, "w");
    if (!out) {
        load_error("could not write output file '%s'\n", outfile);
    }
    for (int i = 0; i < instr_count; i++) {
