...
int tokenize_line(const char *line, Token *tokens, int max_tokens) {
    ...
    while (*p != '\0' && n < max_tokens) {
        ...
        const char *temp = p;
        while (*temp != '\0' && n < max_tokens)
        {
            if(*temp == 'v'){         
                if (*p == '=') { tokens[n].type = TOK_ASSIGN; n++; p++; continue; }
                if (*p == '+') { tokens[n].type = TOK_VEC_PLUS;   n++; p++; continue; }
                if (*p == '-') { tokens[n].type = TOK_VEC_MINUS;  n++; p++; continue; }
                if (*p == '*') { tokens[n].type = TOK_VEC_STAR;   n++; p++; continue; }
                if (*p == '[') { tokens[n].type = TOK_VEC_LBRACKET; n++; p++; continue; }
                if (*p == ']') { tokens[n].type = TOK_VEC_RBRACKET; n++; p++; continue; }
            ...
            if (isalpha((unsigned char)*p)) {
                ...
                if (buf[0] == 'v' && isdigit((unsigned char)buf[1])) {
                    int regnum = atoi(buf + 1);
                    if (regnum < 0 || regnum > 255) return -1;
                    tokens[n].type = TOK_IDENT;
                    tokens[n].value = regnum;
...
static int parse_arith(Token *toks, int n, IRInstr *ir) {
    ...
    switch (toks[1].type) {
        ...
        case TOK_VEC_PLUS: ir->type = IR_VEC_ADD; break;
        case TOK_VEC_MINUS: ir->type = IR_VEC_SUB; break;
        case TOK_VEC_STAR: ir->type = IR_VEC_MUL; break;
...
static int parse_line_pass2(Token *toks, int n, int line_no, IRInstr *ir) {
    ...
    if (toks[0].type == TOK_VEC_LBRACKET) {
        /* [ (IDENT|NUMBER) ] = IDENT   -- memory write, bracket form */
        if (n != 5) return -1;
        if (toks[2].type != TOK_VEC_RBRACKET || toks[3].type != TOK_ASSIGN) return -1;
        if (toks[4].type != TOK_IDENT) return -1;

        ir->type = IR_VEC_MEMWRITE;
        if (toks[1].type == TOK_IDENT) {
            ir->dest = toks[1].value;
            ir->addr_is_reg = 1;
        } else if (toks[1].type == TOK_NUMBER) {
    ...
    if (toks[0].type == TOK_IDENT && n >= 2 && toks[1].type == TOK_ASSIGN) {
        ...
        /* IDENT = [ (IDENT|NUMBER) ]   -- memory read for vector  */
        if (n == 5 && toks[2].type == TOK_VEC_LBRACKET && toks[4].type == TOK_RBRACKET) {
            ir->type = IR_VEC_MEMREAD;
            ir->dest = dest;
            if (toks[3].type == TOK_IDENT) {
                ir->src2 = toks[3].value;
                ir->src2_is_reg = 1;
...
static int encode_instruction(IRInstr *ir, unsigned char out[4]) {
    ...
    switch (ir->type) {
        ...
        case IR_WRITE_LEGACY:
            ...
            break;
        case IR_VEC_ADD:
            opcode = ir->src2_is_reg ? OP_ADD_VEC_VAR : OP_ADD_VEC_CONST;
            dest = ir->dest; op1 = ir->src1; op2 = ir->src2;
            break;
        case IR_VEC_SUB:
            opcode = ir->src2_is_reg ? OP_SUB_VEC_VAR : OP_SUB_VEC_CONST;
            dest = ir->dest; op1 = ir->src1; op2 = ir->src2;
            break;
        case IR_VEC_MUL:
            opcode = ir->src2_is_reg ? OP_MUL_VEC_VAR : OP_MUL_VEC_CONST;
            dest = ir->dest; op1 = ir->src1; op2 = ir->src2;
            break;
        case IR_VEC_MEMREAD:
            opcode = ir->src2_is_reg ? OP_MEMREAD_VEC_VAR : OP_MEMREAD_VEC_CONST;
            dest = ir->dest; op1 = 0; op2 = ir->src2;
            break;
        case IR_VEC_MEMWRITE:
            opcode = ir->addr_is_reg ? OP_MEMWRITE_VEC_VAR : OP_MEMWRITE_VEC_CONST;
            dest = ir->dest; op1 = 0; op2 = ir->src2;
            break;
        case IR_BRANCH: {
