...
int Vec_Register[NUM_VECTOR_REGISTERS][256];
int PC;
...
#define OP_ADD_VEC_VAR    0x21  
#define OP_SUB_VEC_VAR    0x22  
#define OP_MUL_VEC_VAR    0x23  
#define OP_MEMREAD_VEC_VAR    0x25  
#define OP_MEMWRITE_VEC_VAR    0x26  
...
#define OP_ADD_VEC_CONST  0x29  
#define OP_SUB_VEC_CONST  0x2A  
#define OP_MUL_VEC_CONST  0x2B  
#define OP_MEMREAD_VEC_CONST  0x2C  
#define OP_MEMWRITE_VEC_CONST  0x2E  
...
static void do_vec_add_var(int a[],int b[]){
    for (int i = 0; i < 256; i++)
    {
        Vec_Register[dest][i] = a[i] +b[i];
        update_zn(Vec_Register[dest][i]);
        unsigned int ua = (unsigned int)a[i], ub = (unsigned int)b[i], ur = (unsigned int)Vec_Register[dest][i];
        C = (ur > ua || ur<ub) ? 1 : 0;
        int sign_a = (a[i] >> 31) & 1, sign_b = (b[i] >> 31) & 1, sign_r = (Vec_Register[dest][i] >> 31) & 1;
        V = (sign_a != sign_b && sign_r == sign_b) ? 1 : 0;
    }
}
static void do_vec_add_const(int a[],int b){
    for (int i = 0; i < 256; i++)
    {
        Vec_Register[dest][i] = a[i] +b;
        update_zn(Vec_Register[dest][i]);
        unsigned int ua = (unsigned int)a[i], ub = (unsigned int)b, ur = (unsigned int)Vec_Register[dest][i];
        C = (ur > ua || ur<ub) ? 1 : 0;

        int sign_a = (a[i] >> 31) & 1, sign_b = (b>> 31) & 1, sign_r = (Vec_Register[dest][i] >> 31) & 1;
        V = (sign_a != sign_b && sign_r == sign_b) ? 1 : 0;
    }
}

static void do_vec_sub_var(int a[],int b[]){
    for (int i = 0; i < 256; i++)
    {
        Vec_Register[dest][i] = a[i] -b[i];
        update_zn(Vec_Register[dest][i]);
...
static void do_vec_sub_const(int a[],int b){
    for (int i = 0; i < 256; i++)
    {
        Vec_Register[dest][i] = a[i] -b;
        update_zn(Vec_Register[dest][i]);
...
static void do_vec_mul_var(int a[],int b[]){
    for (int i = 0; i < 256; i++)
    {
        Vec_Register[dest][i] = a[i] * b[i];
...
static void do_vec_mul_const(int dest[],int a[],int b){
    for (int i = 0; i < 256; i++)
    {
        dest[i] = a[i] * b;
    }
...
static void do_vec_memread_const(int dest[],int addr){
    for (int i = 0,temp=addr; i < 256; i++,temp=temp+4){
        dest[i] = mem_read32(temp);
    }

}
static void do_vec_memread_var(int dest[],int addr[]){
    for (int i = 0; i < 256; i++){
        dest[i] = mem_read32(addr[i]);
    }
}

static void do_vec_memwrite_var(int dest[],int source[]){
     for (int i = 0; i < 256; i++){
        mem_write32(dest[i],source[i]);
    }
}
static void do_vec_memwrite_const(int dest,int source[]){
     for (int i = 0; i < 256; i++){
        mem_write32(dest,source[i]);
    }
}
...
void execute(void) {
    ...
    switch (opcode) {
        case OP_ADD_VAR:
            ...
            break;
        case OP_ADD_VEC_VAR:
            do_vec_add_var(Vec_Register[src1], Vec_Register[src2]);
            break;
        case OP_ADD_CONST:
            do_add(Register[src1], src2);
            break;
        case OP_ADD_VEC_CONST:
            do_vec_add_const(Vec_Register[src1], src2);
            break;
        case OP_SUB_VAR:
            do_sub(Register[src1], Register[src2]);
            break;
        case OP_SUB_VEC_VAR:
            do_vec_sub_var(Vec_Register[src1], Vec_Register[src2]);
            break;
        case OP_SUB_CONST:
            do_sub(Register[src1], src2);
            break;
        case OP_SUB_VEC_CONST:
            do_vec_sub_const(Vec_Register[src1], src2);
            break;
        case OP_MUL_VAR:
            Register[dest] = Register[src1] * Register[src2];
            break;
        case OP_MUL_VEC_VAR:
            do_vec_mul_var(Vec_Register[src1], Vec_Register[src2]);
            break;
        case OP_MUL_CONST:
            Register[dest] = Register[src1] * src2;
            break;
        case OP_MUL_VEC_CONST:
            do_vec_mul_const(Vec_Register[dest],Vec_Register[src1], src2);
            break;
        case OP_DIV_VAR:
        ...
        case OP_MEMREAD_VAR:
            ...
            break;
        case OP_MEMREAD_VEC_VAR:
            do_vec_memread_var(Vec_Register[dest], Vec_Register[src2]);
            break;
        case OP_MEMREAD_CONST:
            Register[dest] = mem_read32(src2);
            break;
        case OP_MEMREAD_VEC_CONST:
            do_vec_memread_const(Vec_Register[dest], src2);
            break;
        case OP_MEMWRITE_VAR:
            mem_write32(Register[dest], Register[src2]);
            break;
        case OP_MEMWRITE_VEC_VAR:
            do_vec_memwrite_var(Vec_Register[dest], Vec_Register[src2]);
            break;
        case OP_MEMWRITE_CONST:
            mem_write32(dest, Register[src2]);
            break;
        case OP_MEMWRITE_VEC_CONST:
            do_vec_memwrite_const(dest, Vec_Register[src2]);
            break;
        default:
            if (is_branch) {
